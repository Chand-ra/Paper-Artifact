...
// Must match STARTUP_COMPLETE_MARKER in fuzzln/src/runners.rs.
#define STARTUP_COMPLETE_MARKER "/tmp/fuzzln-startup-complete"
...
#else
  int fd = open("/tmp/fuzzln-crash.log", O_WRONLY | O_CREAT | O_TRUNC, 0644);
  if (fd >= 0) {
    write(fd, buf, len);
    close(fd);
  }
