...
use fuzzln::bitcoin::{BitcoinCli, TxBlockPosition, Utxo};
use fuzzln::bolt::{
...
use fuzzln::channel_tx::{
    ChannelConfig, ChannelPartyConfig, ChannelState, FundingTransaction, HolderIdentity, Side,
    build_funding_transaction,
};
use fuzzln::noise::{ConnectionError, NoiseConnection};
use fuzzln::oracles::{AcceptChannelContext, AcceptChannelOracle, Oracle};
use fuzzln::pending_channel::PendingChannel;
use fuzzln::violation::Violation;
use fuzzln_ir::operation::AcceptChannelField;
use fuzzln_ir::{Operation, Program, Variable};
...
pub enum ExecuteError {
    ...
    #[error("connection: {0}")]
    Connection(#[from] fuzzln::noise::ConnectionError),

    /// Failed to decode a received message.
    #[error("decode: {0}")]
    Decode(#[from] fuzzln::bolt::BoltError),
    ...
    PeerError(fuzzln::bolt::Error),

    /// Wallet UTXOs could not cover the funding amount and fees.
    #[error("funding: {0}")]
    InsufficientFunds(#[from] fuzzln::channel_tx::InsufficientFunds),

    /// Failed to construct the initial commitment state.
    #[error("commitment: {0}")]
    Commitment(#[from] fuzzln::channel_tx::CommitmentError),
...
mod tests {
    ...
    use fuzzln::bolt::{AcceptChannelTlvs, GossipTimestampFilter, Init, Ping};
    use fuzzln_ir::Instruction;
    use fuzzln_ir::operation::ShutdownScriptVariant;
    ...
    fn execute_recv_peer_error() {
        let peer_error = fuzzln::bolt::Error::all_channels("Wrong channel id in channel_ready");
    ...
    fn execute_send_funding_created_push_exceeds_funding() {
        ...
        assert!(matches!(
            err,
            ExecuteError::Commitment(fuzzln::channel_tx::CommitmentError::PushExceedsFunding)
        ));
    }
    ...
    fn execute_send_funding_created_funding_msat_overflow() {
        ...
        assert!(matches!(
            err,
            ExecuteError::Commitment(fuzzln::channel_tx::CommitmentError::FundingMsatOverflow)
        ));
    }
