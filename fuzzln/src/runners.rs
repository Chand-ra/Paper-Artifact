...
//! [`StdRunner`] auto-selects between them based on the `FUZZLN_NYX`
//! environment variable.

#[cfg(feature = "nyx")]
use fuzzln_nyx_sys::{nyx_fail, nyx_get_fuzz_input, nyx_init, nyx_release, nyx_skip};
...
/// node startup.
const STARTUP_COMPLETE_MARKER: &str = "/tmp/fuzzln-startup-complete";

fn create_startup_complete_marker() {
    std::fs::File::create(STARTUP_COMPLETE_MARKER).expect("startup complete file created");
}

/// `Runner` provides an abstraction for a fuzzln test case runner (e.g. run under nyx,
...
/// `LocalRunner` reads fuzz input from the `FUZZLN_INPUT` environment variable path
...
impl Runner for LocalRunner {
    ...
    fn get_fuzz_input(&self) -> Vec<u8> {
        ...
        if let Ok(path) = std::env::var("FUZZLN_INPUT") {
...
/// `StdRunner` automatically selects `NyxRunner` when the `FUZZLN_NYX` environment
...
impl Runner for StdRunner {
    fn new() -> Self {
        #[cfg(feature = "nyx")]
        if std::env::var("FUZZLN_NYX").is_ok() {
