...
/// `FUZZLN_NYX` and `FUZZLN_NYX_LOG` are set, `nyx_log` is installed instead.
fn init_logging() {
    #[cfg(feature = "nyx")]
    if std::env::var("FUZZLN_NYX").is_ok() && std::env::var("FUZZLN_NYX_LOG").is_ok() {
        crate::nyx_log::init();
        return;
    }
...
///
/// This is the main entry point for fuzzln scenario binaries. It initializes
...
#[must_use]
pub fn fuzzln_run<S: Scenario>() -> std::process::ExitCode {
    ...
    if std::env::var("FUZZLN_NYX").is_ok() {
        std::panic::set_hook(Box::new(|info| {
            ...
            unsafe {
                fuzzln_nyx_sys::nyx_fail(c_message.as_ptr());
            }
        }));
    }
