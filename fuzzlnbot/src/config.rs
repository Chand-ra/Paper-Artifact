...
/// A parsed and validated fuzzlnbot campaign configuration.
...
pub struct CampaignConfig {
    ...
    /// Path to the fuzzln repository root.
    ...
    /// Nyx shared directory path; created automatically by `fuzzlnbot start`.
...
impl CampaignConfig {
    ...
    pub fn image_tag(&self) -> String {
        self.image
            .clone()
            .unwrap_or_else(|| format!("fuzzln-{}-{}", self.target, self.scenario))
    }

    /// Returns the path to the fuzzln-ir custom mutator shared library.
    #[must_use]
    pub fn ir_mutator_path(&self) -> PathBuf {
        self.fuzzln_dir
            .join("target")
            .join("release")
            .join("libfuzzln_ir_mutator.so")
    }
    ...
    pub fn check_paths(&self) -> Vec<String> {
        ...
        if !self.fuzzln_dir.exists() {
            errors.push(format!(
                "fuzzln_dir does not exist: {}",
                self.fuzzln_dir.display()
            ));
        }
...
fuzzln_dir = "."
runners = 8
seed_dir = "/tmp/fuzzln-seeds"
output_dir = "/tmp/fuzzln-out"
sharedir = "/tmp/fuzzln-nyx"
"#;
    ...
    fn load_parses_valid_config() {
        ...
        assert_eq!(config.fuzzln_dir, PathBuf::from("."));
        assert_eq!(config.runners, 8);
        assert_eq!(
            config.seed_dir.as_deref(),
            Some(Path::new("/tmp/fuzzln-seeds"))
        );
        assert_eq!(config.output_dir, PathBuf::from("/tmp/fuzzln-out"));
        assert_eq!(config.sharedir, PathBuf::from("/tmp/fuzzln-nyx"));
    ...
    fn image_tag_derives_default_from_target_and_scenario() {
        ...
        assert_eq!(config.image_tag(), "fuzzln-lnd-encrypted_bytes");
    }
    ...
    fn load_accepts_missing_seed_dir() {
        let dir = tempfile::tempdir().unwrap();
        let content = VALID_CONFIG.replace("seed_dir = \"/tmp/fuzzln-seeds\"\n", "");
        let path = write_config(dir.path(), &content);
    ...
    fn check_paths_reports_missing_seed_dir() {
        ...
        let content = VALID_CONFIG.replace("/tmp/fuzzln-seeds", &missing.display().to_string());
        let path = write_config(dir.path(), &content);
        let config = CampaignConfig::load(&path).unwrap();
    ...
    fn check_paths_reports_empty_seed_dir() {
        ...
        let content = VALID_CONFIG.replace("/tmp/fuzzln-seeds", &seed.display().to_string());
        let path = write_config(dir.path(), &content);
        let config = CampaignConfig::load(&path).unwrap();
    ...
    fn check_paths_accepts_non_empty_seed_dir() {
        ...
        let content = VALID_CONFIG
            .replace("/tmp/fuzzln-seeds", &seed.display().to_string())
            .replace("/home/user/AFLplusplus", &dir.path().display().to_string())
            .replace(
                "fuzzln_dir = \".\"",
                &format!("fuzzln_dir = \"{}\"", dir.path().display()),
            );
        let path = write_config(dir.path(), &content);
        let config = CampaignConfig::load(&path).unwrap();
    ...
    fn check_paths_skips_seed_dir_when_absent() {
        let dir = tempfile::tempdir().unwrap();
        let content = VALID_CONFIG
            .replace("seed_dir = \"/tmp/fuzzln-seeds\"\n", "")
            .replace("/home/user/AFLplusplus", &dir.path().display().to_string())
            .replace(
                "fuzzln_dir = \".\"",
                &format!("fuzzln_dir = \"{}\"", dir.path().display()),
