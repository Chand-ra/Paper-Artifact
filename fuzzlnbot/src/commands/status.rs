...
/// Command handler for `fuzzlnbot status`.
pub struct StatusCommand;

/// CLI arguments for `fuzzlnbot status`.
#[derive(Debug, Args)]
pub struct StatusArgs {
    /// Campaign ID to inspect (a directory name under `~/.fuzzlnbot/runs`).
    campaign_id: String,
...
///
/// A bash loop re-runs `fuzzlnbot status <id> --summary` every [`RELOAD_SECS`]
...
fn dashboard_command(campaign_id: &str) -> String {
    // Explicit path to this binary rather than a bare `fuzzlnbot`, so the window
    // runs the same binary the user invoked, not whatever is on PATH.
    let exe = std::env::current_exe()
        .map_or_else(|_| "fuzzlnbot".to_string(), |p| p.display().to_string());
...
/// mean a fuzzlnbot bug (wrong key or corrupt file), not runtime variability, so
