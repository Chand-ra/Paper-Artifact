...
/// Command handler for `fuzzlnbot doctor`.
pub struct DoctorCommand;

/// CLI arguments for `fuzzlnbot doctor`.
#[derive(Debug, Args)]
...
impl DoctorInputs {
    ...
    /// provided; `--fuzzln-dir` defaults to the current directory.
...
impl DoctorCommand {
    ...
    pub fn execute(args: &DoctorArgs) -> bool {
        ...
        let fuzzln_dir = &inputs.fuzzln_dir;
        ...
        for script in REQUIRED_SCRIPTS {
            let path = fuzzln_dir.join("scripts").join(script);
            checks.push(DoctorCheck::new(
        ...
        for dockerfile in REQUIRED_DOCKERFILES {
            let path = fuzzln_dir.join(dockerfile);
            checks.push(DoctorCheck::new(
...
fn print_human_report(report: &DoctorReport) {
    ...
    if report.overall {
        println!("\nfuzzlnbot doctor: all {total} checks passed");
    } else {
        let failed = report.checks.iter().filter(|check| !check.passed).count();
        println!("\nfuzzlnbot doctor: {failed} of {total} checks failed");
    }
}
...
mod tests {
    ...
    #[test]
    fn resolve_without_config_overrides_fuzzln_dir() {
        let mut args = sample_doctor_args();
        args.aflpp_path = Some(PathBuf::from("/opt/aflpp"));
        args.fuzzln_dir = Some(PathBuf::from("/custom/fuzzln"));

        let inputs = DoctorInputs::resolve(None, &args);

        assert_eq!(inputs.fuzzln_dir, Path::new("/custom/fuzzln"));
    }
