...
//! serializes them (see `fuzzln-ir-mutator`). This command decodes one and
...
use clap::Args;
use fuzzln_ir::Program;

/// Command handler for `fuzzlnbot print-ir`.
pub struct PrintIrCommand;

/// CLI arguments for `fuzzlnbot print-ir`.
#[derive(Debug, Args)]
...
mod tests {
    use super::*;
    use fuzzln_ir::{Instruction, Operation};

    /// Postcard is how fuzzln IR programs are serialized in the fuzzing loop.
