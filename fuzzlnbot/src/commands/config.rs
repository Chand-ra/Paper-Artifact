...
/// Command handler for `fuzzlnbot config`.
pub struct ConfigCommand;

/// CLI arguments for `fuzzlnbot config`.
#[derive(Debug, Args)]
...
/// JSON-serializable report for `fuzzlnbot config --json`.
            ...
            Ok(config) => {
                ...
                if args.json {
                    let report = ConfigReport {
                        ...
                        fuzzln_dir: config.fuzzln_dir.display().to_string(),
                    ...
                    println!("fuzzln_dir:  {}", config.fuzzln_dir.display());
