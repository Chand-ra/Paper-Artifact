...
/// Command handler for `fuzzlnbot corpus`.
pub struct CorpusCommand;

/// CLI arguments for `fuzzlnbot corpus`.
#[derive(Debug, Args)]
...
/// CLI arguments for `fuzzlnbot corpus merge`.
#[derive(Debug, Args)]
pub struct MergeArgs {
    /// Campaign IDs whose runner queues to merge (directories under `~/.fuzzlnbot/runs`).
    #[arg(required = true)]
...
/// CLI arguments for `fuzzlnbot corpus minimize`.
#[derive(Debug, Args)]
pub struct MinimizeArgs {
    ...
    #[arg(short = 'i', long)]
    input: Vec<PathBuf>,
    /// Output directory (default: `~/.fuzzlnbot/runs/<id>/corpus-min/`).
    #[arg(short = 'o', long)]
...
mod tests {
    ...
    fn stage_inputs_shortens_long_input_names() {
        ...
        // A fuzzln-ir queue filename near NAME_MAX (255). afl-cmin -X would prefix
