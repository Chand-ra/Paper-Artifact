...
/// Command handler for `fuzzlnbot build`.
pub struct BuildCommand;

/// CLI arguments for `fuzzlnbot build`.
#[derive(Debug, Args)]
pub struct BuildArgs {
    ...
    #[arg(long)]
    coverage: bool,
    /// Docker image tag; overrides the config value and the default fuzzln naming convention.
    #[arg(long)]
    image: Option<String>,
    /// Path to fuzzln repository root; overrides the config value.
    #[arg(long)]
    fuzzln_dir: Option<PathBuf>,
...
impl BuildInputs {
    ...
    pub fn from_config(config: &CampaignConfig, image: &str) -> Self {
        Self {
            ...
            dockerfile: config
                ...
                .join("Dockerfile"),
            fuzzln_dir: config.fuzzln_dir.clone(),
    ...
    /// file is provided; `--fuzzln-dir` defaults to the current directory.
    fn resolve(config: Option<&CampaignConfig>, args: &BuildArgs) -> Self {
        ...
        let fuzzln_dir = args
            .fuzzln_dir
            .clone()
            .or_else(|| config.map(|c| c.fuzzln_dir.clone()))
...
///
/// Shared by `fuzzlnbot build` and `fuzzlnbot start`.
...
mod tests {
    ...
    fn sample_config() -> CampaignConfig {
        CampaignConfig {
            ...
            fuzzln_dir: PathBuf::from("/repo/fuzzln"),
            runners: 1,
            seed_dir: None,
    ...
    fn resolve_uses_config_values_by_default() {
        ...
        assert_eq!(inputs.image, "fuzzln-ldk-init");
        assert_eq!(
            inputs.dockerfile,
            Path::new("/repo/fuzzln/workloads/ldk/Dockerfile")
        );
        assert_eq!(inputs.fuzzln_dir, Path::new("/repo/fuzzln"));
    ...
    fn resolve_overrides_target_and_scenario() {
        ...
        assert_eq!(inputs.image, "fuzzln-cln-noise");
        assert_eq!(
            inputs.dockerfile,
            Path::new("/repo/fuzzln/workloads/cln/Dockerfile")
        );
    }
    ...
    fn resolve_selects_expected_dockerfile_for_each_target() {
        let config = sample_config();
        let cases = [
            (Target::Lnd, "/repo/fuzzln/workloads/lnd/Dockerfile"),
            (Target::Cln, "/repo/fuzzln/workloads/cln/Dockerfile"),
            (Target::Ldk, "/repo/fuzzln/workloads/ldk/Dockerfile"),
            (Target::Eclair, "/repo/fuzzln/workloads/eclair/Dockerfile"),
        ];
    ...
    fn resolve_supports_coverage_and_custom_image() {
        ...
        assert_eq!(
            inputs.dockerfile,
            Path::new("/repo/fuzzln/workloads/eclair/Dockerfile.coverage")
        );
        assert!(inputs.no_cache);
    }
    ...
    fn resolve_uses_default_coverage_image_when_not_overridden() {
        ...
        assert_eq!(inputs.image, "fuzzln-cln-noise-coverage");
        assert_eq!(
            inputs.dockerfile,
            Path::new("/repo/fuzzln/workloads/cln/Dockerfile.coverage")
        );
    }

    #[test]
    fn resolve_overrides_fuzzln_dir() {
        let config = sample_config();
        let mut args = sample_build_args();
        args.fuzzln_dir = Some(PathBuf::from("/tmp/local-fuzzln"));

        let inputs = BuildInputs::resolve(Some(&config), &args);

        assert_eq!(inputs.fuzzln_dir, Path::new("/tmp/local-fuzzln"));
        assert_eq!(
            inputs.dockerfile,
            Path::new("/tmp/local-fuzzln/workloads/ldk/Dockerfile")
        );
    }
    ...
    fn resolve_without_config_uses_cli_args() {
        ...
        assert_eq!(inputs.image, "fuzzln-lnd-encrypted_bytes");
        assert_eq!(inputs.dockerfile, Path::new("./workloads/lnd/Dockerfile"));
        assert_eq!(inputs.fuzzln_dir, Path::new("."));
