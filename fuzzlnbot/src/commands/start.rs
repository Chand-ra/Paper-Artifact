...
/// Command handler for `fuzzlnbot start`.
pub struct StartCommand;

/// CLI arguments for `fuzzlnbot start`.
#[derive(Debug, Args)]
...
impl StartCommand {
    ...
    pub fn execute(args: &StartArgs) -> bool {
        ...
        if !tmux::is_available() {
            log::error!("tmux is not available; install it or run `fuzzlnbot doctor` for details");
            return false;
        }
        ...
        if let Some(stats) = existing_campaign_runner(&config.output_dir, config.runners) {
            log::error!(
                "output_dir already holds a campaign ({}); fuzzlnbot start only \
        ...
        if config.scenario.starts_with("ir") && !build_ir_mutator(&config.fuzzln_dir) {
            return false;
        }
        ...
        let Some(git_hash) = fuzzln_git_hash(&config.fuzzln_dir) else {
            log::error!("could not determine fuzzln git hash");
            return false;
        };
...
fn setup_nyx(config: &CampaignConfig, image: &str) -> bool {
    let script = config.fuzzln_dir.join("scripts").join("setup-nyx.sh");
    if !script.exists() {
...
fn verify_startup(
    ...
    // remain None and `fuzzlnbot stop` reads them from tmux pane PIDs at teardown.
...
/// Returns the environment variables needed for the fuzzln-ir custom mutator.
///
...
/// Builds the fuzzln-ir-mutator shared library in release mode.
fn build_ir_mutator(fuzzln_dir: &Path) -> bool {
    log::info!("building IR mutator library");
    let status = match Command::new("cargo")
        .args(["build", "--release", "-p", "fuzzln-ir-mutator"])
        .current_dir(fuzzln_dir)
        .status()
    {
        Ok(status) => status,
        Err(e) => {
    ...
    if !status.success() {
        log::error!("cargo build for fuzzln-ir-mutator failed with {status}");
        return false;
    }
...
/// Returns the fuzzln repository git hash, or `None` if not a git repo.
fn fuzzln_git_hash(fuzzln_dir: &Path) -> Option<String> {
    command_stdout(
        Command::new("git")
            .arg("-C")
            .arg(fuzzln_dir)
...
"#,
        ...
        assert!(envs[0].1.ends_with("libfuzzln_ir_mutator.so"));
