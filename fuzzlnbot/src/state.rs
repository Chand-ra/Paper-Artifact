...
//! `~/.fuzzlnbot/runs/<campaign-id>/state.json`. The `stop` and `status`
...
pub enum Status {
    ...
    Failed,
    /// Campaign was torn down by `fuzzlnbot stop`.
    Stopped,
}
...
impl CampaignState {
    ...
    /// Returns the base directory for all campaign state: `~/.fuzzlnbot/runs`.
    ///
    /// Returns `None` if the home directory cannot be determined.
    pub fn runs_dir() -> Option<PathBuf> {
        std::env::home_dir().map(|home| home.join(".fuzzlnbot").join("runs"))
    }
...
mod tests {
    ...
    fn sample_state() -> CampaignState {
        CampaignState {
            ...
            image: "fuzzln-lnd-encrypted_bytes".to_string(),
            image_digest: "sha256:abc123".to_string(),
            output_dir: PathBuf::from("/tmp/fuzzln-out"),
            sharedir: PathBuf::from("/tmp/fuzzln-nyx"),
            fuzzln_git_hash: "deadbeef".to_string(),
...
"#,
        ...
        let state = CampaignState::new(
            ...
            &config,
            "fuzzln-lnd-encrypted_bytes".to_string(),
        ...
        assert_eq!(state.image, "fuzzln-lnd-encrypted_bytes");
        assert_eq!(state.image_digest, "sha256:abc123");
        assert_eq!(state.fuzzln_git_hash, "deadbeef");
