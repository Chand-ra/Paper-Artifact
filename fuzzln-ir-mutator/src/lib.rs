...
//! - `AFL_CUSTOM_MUTATOR_LIBRARY=/path/to/libfuzzln_ir_mutator.so`
...
//! - `RUST_LOG=fuzzln_ir_mutator::trim=debug` -- print the decoded `Program`
...
fn warn_on_unset_afl_env() {
    ...
    let bar = "=".repeat(72);
    eprintln!("\n{bar}");
    eprintln!("[fuzzln-ir-mutator] WARNING: required AFL++ env vars are not set to 1:");
    for name in &unset {
        eprintln!("  - {name}");
    }
...
/// `RUST_LOG=fuzzln_ir_mutator::trim=debug` to see trim before/after dumps.
///
/// # Safety
///
...
pub unsafe extern "C" fn afl_custom_init_trim(
    ...
    let before = log::log_enabled!(target: "fuzzln_ir_mutator::trim", log::Level::Debug)
        .then(|| program.clone());
...
pub unsafe extern "C" fn afl_custom_describe(
    ...
    state.description.extend_from_slice(b"fuzzln-ir:");
...
#[cfg(test)]
mod tests {
    use fuzzln_ir::generators::OpenChannelGenerator;
    use std::ffi::CStr;
    use std::ptr;
    ...
    fn verify_fresh_generation(state: &State) {
        ...
        let s = unsafe { CStr::from_ptr(ptr) }
            .to_str()
            .expect("valid utf-8");
        assert_eq!(s, "fuzzln-ir:fresh", "fresh generation did not occur");
    }
    ...
    fn reducible_seed_bytes() -> Vec<u8> {
        ...
        program.instructions.push(fuzzln_ir::Instruction {
            operation: fuzzln_ir::Operation::LoadAmount(0xdead_beef),
            inputs: vec![],
        });
    ...
    fn describe_lists_stacked_mutator_sequence() {
        ...
        for _ in 0..10 {
            ...
            let s = unsafe { CStr::from_ptr(ptr) }
                ...
                .expect("valid utf-8");
            let suffix = s
                .strip_prefix("fuzzln-ir:")
                .expect("fuzzln-ir: prefix on description");
            if suffix == "fresh" {
                continue;
            }
