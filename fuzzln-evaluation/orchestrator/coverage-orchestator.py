#!/usr/bin/env python3
"""
FuzzLN Coverage Campaign Orchestrator
...
`fuzzlnbot` is used for one-time setup steps (`fuzzlnbot build` to build Docker workload
images, and `fuzzlnbot doctor` to validate the host). Similar to `fuzzlnbot start`, this
script handles building `libfuzzln_ir_mutator.so`, preparing the Nyx sharedir, and
...
Requirements:
    pip install rich
    fuzzlnbot (must be available in your PATH: `cargo install --path fuzzlnbot`)
...
Usage:
    python fuzzln-orchestrator.py \
    --out-dir OUT_DIR \
    --configs LABEL:FUZZLN_DIR[,LABEL:FUZZLN_DIR...] \
    --scenario SCENARIO \
...
Examples:
    ...
    python fuzzln-orchestrator.py \
        ...
        --configs baseline:~/fuzzln,experimental:~/fuzzln-new-mutator \
        --scenario ir \
    ...
    python fuzzln-orchestrator.py \
        --out-dir ./eval-results \
        --configs control:~/fuzzln,test:~/fuzzln-new-mutator \
        --scenario ir \
        --targets cln \
        --cores 4,5,6,7,8 \
        --trials 5 \
...
def testcache_size_mb() -> Optional[int]:
    ...
    Mirrors fuzzlnbot's conservative thresholds (50/250/500 MB), since
...
class TrialConfig:
    ...
    def image_tag(self) -> str:
        """Docker image tag: fuzzln-<label>-<target>-<scenario>."""
        return f"fuzzln-{self.label}-{self.target}-{self.scenario}"
    ...
    def ir_mutator_path(self) -> Path:
        return self.fuzzln_dir / "target" / "release" / "libfuzzln_ir_mutator.so"
    ...
    def build_afl_env(self) -> dict:
        ...
        # Matches fuzzlnbot's ir_mutator_envs(): only for scenarios starting with "ir".
...
class DashboardRenderer:
    ...
    def render(cls, state: CampaignState) -> Group:
        ...
        core_table = Table(
            title=f"FuzzLN Orchestrator ([bold yellow]Coverage Mode[/])  [cyan]{completed}[/]/[cyan]{total}[/]"
...
class EnvironmentManager:
    ...
    `fuzzlnbot doctor` preflight, required-file checks, Docker image builds for every
    (config, target) pair, and (if needed) the IR mutator build."""

    @staticmethod
    def validate(afl_dir: Path, fuzzln_dirs: dict, console: Console):
        """Abort early if `fuzzlnbot` is missing or its own doctor checks fail."""
        if not shutil.which("fuzzlnbot"):
            sys.exit(
                "ERROR: 'fuzzlnbot' not found in PATH. Install via `cargo install --path fuzzlnbot`."
            )

        for label, fuzzln_dir in fuzzln_dirs.items():
            console.print(f"[bold cyan]Running fuzzlnbot doctor for '{label}'...[/]")
            res = subprocess.run(
                [
                    "fuzzlnbot",
                    "doctor",
            ...
            try:
                ...
                if not data.get("overall"):
                    ...
                    sys.exit(1)
                console.print("[bold green]fuzzlnbot doctor checks passed![/]\n")
            except Exception:
                sys.exit(
    ...
    @staticmethod
    def validate_paths(afl_dir: Path, fuzzln_dirs: dict, console: Console):
        """Check the required execution files so a missing one fails fast instead of mid-campaign."""
        required = {
            "afl-fuzz binary": afl_dir / "afl-fuzz",
        }
        for label, fuzzln_dir in fuzzln_dirs.items():
            required[f"setup-nyx.sh ({label})"] = fuzzln_dir / "scripts" / "setup-nyx.sh"
    ...
    def build_docker_images(
        targets: list[str], scenario: str, fuzzln_dirs: dict, console: Console
    ):
        """Build every distinct (label, target) Docker image up front."""
        console.print("[bold cyan]Pre-building isolated Docker images…[/]")
        for label, fuzzln_dir in fuzzln_dirs.items():
            for tgt in targets:
                default_tag = f"fuzzln-{tgt}-{scenario}"
                isolated_tag = f"fuzzln-{label}-{tgt}-{scenario}"

                console.print(f"\n[bold]Building {isolated_tag} from {fuzzln_dir}[/]")
                if (
                    subprocess.run(
                        [
                            "fuzzlnbot",
    ...
    @staticmethod
    def build_ir_mutator(fuzzln_dirs: dict, console: Console):
        """Builds the fuzzln-ir-mutator shared library in release mode once, up front,
        if the scenario needs it (similar to how `fuzzlnbot start` does it)."""
        for label, fuzzln_dir in fuzzln_dirs.items():
            console.print(
                f"[bold cyan]Building fuzzln-ir-mutator (release) for '{label}'...[/]"
            )
            if (
                subprocess.run(
                ...
                sys.exit(
                    f"[bold red]cargo build for fuzzln-ir-mutator FAILED in {fuzzln_dir}[/]"
                )
...
class TrialRunner:
    ...
    def ensure_sharedir(self):
        ...
        script = self.cfg.fuzzln_dir / "scripts" / "setup-nyx.sh"
...
def worker_thread(core: int, work: Queue, args, state: CampaignState, fuzzln_dirs: dict):
...
def ensure_seed_dir(args, targets, console: Console):
    ...
    (similar to how `fuzzlnbot start` handles seed provisioning)."""
    if args.seed_dir:
...
def parse_args():
    ...
    p = argparse.ArgumentParser(
        description="FuzzLN Fuzzing Coverage Campaign Orchestrator"
    )
    p.add_argument("--out-dir", required=True, type=Path)
    p.add_argument("--configs", required=True, help="'label:fuzzln_dir,label:fuzzln_dir'")
...
def main():
    ...
    try:
        for item in args.configs.split(","):
            ...
            l = l.strip()
            labels.append(l)
            fuzzln_dirs[l] = Path(d.strip()).expanduser().resolve()
    except ValueError:
        sys.exit("ERROR: --configs must use 'label:fuzzln_dir' format")

    EnvironmentManager.validate(args.afl_dir, fuzzln_dirs, console)
    EnvironmentManager.validate_paths(args.afl_dir, fuzzln_dirs, console)

    cores = [int(c) for c in args.cores.split(",")]

    EnvironmentManager.build_docker_images(targets, args.scenario, fuzzln_dirs, console)

    if args.scenario.startswith("ir"):
        EnvironmentManager.build_ir_mutator(fuzzln_dirs, console)
    ...
    threads = [
        threading.Thread(
            target=worker_thread, args=(c, work, args, state, fuzzln_dirs), daemon=True
        )
        for c in cores
    ]
    ...
    if state.shutdown.is_set():
        ...
        if len(labels) == 2:
            console.print(
                f"Run: [cyan]python scripts/fuzzln-evaluation.py {args.out_dir} {labels[0]} {labels[1]}[/]"
            )
