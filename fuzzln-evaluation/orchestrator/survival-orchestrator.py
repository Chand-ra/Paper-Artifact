#!/usr/bin/env python3
"""
FuzzLN Survival (TTE) Campaign Orchestrator
...
vulnerable commit. All configs share ONE `--fuzzln-dir` checkout — only the mutator
shared library compiled there differs between ablation runs; compile the variant under
test at `target/release/libfuzzln_ir_mutator.so` before invoking the orchestrator (see
the ablation note below).

`fuzzlnbot doctor` is used for one-time host validation. This script handles preparing the
...
Ablation workflow:
    Compile the mutator variant under test so that `--fuzzln-dir`'s
    `target/release/libfuzzln_ir_mutator.so` is the variant under evaluation, then run
    ...
    shared --fuzzln-dir, so ablations sharing a scenario are run as separate invocations).
...
Examples:
    ...
    # Ablation run: compile the component-a mutator under --fuzzln-dir first, then
    python survival-orchestrator.py \
        --out-dir ./survival-results \
...
class TrialConfig:
    ...
    scenario: str
    out_dir: Path
    fuzzln_dir: Path  # shared across every label/scenario in this run
    afl_dir: Path
    timeout: int
    ...
    def image_tag(self) -> str:
        """Docker image tag: fuzzln-eval-<target>-<bug>-<scenario>.
        ...
        """
        return f"fuzzln-eval-{self.target}-{self.bug.lower()}-{self.scenario}"
...
class ReproductionManager:
    ...
    def _reproduce(self, crash_file: Path, timeout: int = 120) -> tuple[bool, Path]:
        safe_temp_file = self.trial_dir / f"fuzzln_crash_{uuid.uuid4().hex[:8]}.bin"
        shutil.copy(crash_file, safe_temp_file)
        safe_temp_file.chmod(0o644)

        container_name = f"fuzzln-repro-{uuid.uuid4().hex[:12]}"
        try:
...
class DashboardRenderer:
    ...
    def render(cls, state: CampaignState) -> Group:
        ...
        core_table = Table(
            title=f"FuzzLN Orchestrator ([bold yellow]TTE Mode[/])  [cyan]{completed}[/]/[cyan]{total}[/]"
...
class EnvironmentManager:
    ...
    a `fuzzlnbot doctor` preflight, and (for IR scenarios) confirming the mutator .so
    under the shared --fuzzln-dir has already been compiled — this script does not
    build it, since ablations are expected to be recompiled by hand between runs."""

    @staticmethod
    def validate(afl_dir: Path, fuzzln_dir: Path, console: Console):
        if not shutil.which("fuzzlnbot"):
            sys.exit(
                "ERROR: 'fuzzlnbot' not found in PATH. Install via `cargo install --path fuzzlnbot`."
            )

        console.print("[bold cyan]Running fuzzlnbot doctor...[/]")
        res = subprocess.run(
            [
                "fuzzlnbot",
                "doctor",
    ...
    @staticmethod
    def validate_paths(
        afl_dir: Path, fuzzln_dir: Path, scenarios: set[str], console: Console
    ):
        ...
        if any(s.startswith("ir") for s in scenarios):
            mutator = fuzzln_dir / "target" / "release" / "libfuzzln_ir_mutator.so"
            if not mutator.exists():
                missing["libfuzzln_ir_mutator.so"] = mutator
...
class TrialRunner:
    ...
    def cleanup(self):
        ...
        for f in self.cfg.trial_dir.glob("fuzzln_crash_*"):
            try:
...
def parse_args():
    p = argparse.ArgumentParser(
        description="FuzzLN Survival (TTE) Campaign Orchestrator"
    )
    p.add_argument("--out-dir", required=True, type=Path)
    p.add_argument("--configs", required=True, help="'label:scenario,label:scenario'")
    p.add_argument(
        "--fuzzln-dir", required=True, type=Path, help="Shared across all configs"
    )
    ...
    args.fuzzln_dir = args.fuzzln_dir.expanduser().resolve()
...
def main():
    ...
    EnvironmentManager.validate(args.afl_dir, args.fuzzln_dir, console)
    EnvironmentManager.validate_paths(
        args.afl_dir, args.fuzzln_dir, set(scenarios), console
    )
