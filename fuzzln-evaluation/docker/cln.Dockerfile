...
RUN if [ -n "$FLAG_PATCH" ]; then \
        git -C /cln apply "/fuzzln-vulns/$FLAG_PATCH"; \
    fi
...
RUN set -eu; for f in fuzzln-scenarios/src/bin/cln_*.rs; do \
...
COPY --from=builder /fuzzln/target/release/cln_${SCENARIO} /cln-scenario
