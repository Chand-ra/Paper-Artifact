...
RUN if [ -n "$FLAG_PATCH" ]; then \
        git -C /lnd apply "/fuzzln-vulns/$FLAG_PATCH"; \
    fi
...
RUN set -eu; for f in fuzzln-scenarios/src/bin/lnd_*.rs; do \
...
COPY --from=builder /fuzzln/target/release/lnd_${SCENARIO} /lnd-scenario
